...
一般来说，vim和Python是两个不常规的数据整理工具。对于某些复杂的多行转换，vim宏是非常有用的工具。你可以记录一系列操作，并根据需要重复执行多次，例如，在编辑的[讲义](editors.md#宏)(去年 [视频](../2019/editors.md))中，有一个示例是使用vim宏将XML格式的文件转换为JSON。
